...
module PSRAM_QPI (
    ...
    reg         state, nstate;
    reg [7:0]   counter;
    wire[7:0]   CMD_66H = 8'h66;
    localparam  IDLE = 1'b0,
                CMD_IN = 1'b1;
    // Drive the Serial Clock (sck) @ clk/2
    always @*
        case (state)
            IDLE:  if(QPI_en) nstate = CMD_IN; else nstate = IDLE;
            CMD_IN: if(done) nstate = IDLE; else nstate = CMD_IN;
        endcase
    always @ (posedge clk or negedge rst_n)
        if(!rst_n) state <= IDLE;
        else state <= nstate;
    ...
    always @ (posedge clk or negedge rst_n)
        if(!rst_n)
            ...
            counter <= 8'b0;
    assign dout     = {3'b0, CMD_66H[7 - counter]};
    assign douten   = (~ce_n);
    assign done     = (counter == FINAL_COUNT + 1);
endmodule