...
void btrace_write(int b_type, uint32_t pc_now, uint32_t pc_target){
  // FILE *btrace_Write =fopen("outputs/btrace.txt","a");
  // fprintf(btrace_Write,"%d  %x  %x\n", b_type, pc_now, pc_target); 
  // fclose(btrace_Write);
}
...
static int decode_exec(Decode *s) {
  ...
  #define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
    ...
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal  ,J,   s->dnpc = s->pc + imm; btrace_write(0, s->pc, s->dnpc); R(rd) = s->snpc; ftrace_check(0, s, imm, rd));//跳转指令
    INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw  , SW,  Mw(src1 + imm, 4, src2));
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr ,A,   s->dnpc = (src1 + imm) & ~1; btrace_write(1, s->pc, s->dnpc); R(rd) = s->snpc; ftrace_check(1, s, imm, rd));//跳转指令
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq , B,   s->dnpc = (src1 == src2)? s->pc + imm : s->dnpc); btrace_write(2, s->pc, s->dnpc);//跳转指令
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne , B,   s->dnpc = (src1 != src2)? s->pc + imm : s->dnpc); btrace_write(3, s->pc, s->dnpc);//跳转指令
    INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt , B,   s->dnpc = ((int32_t)src1 <  (int32_t)src2)? s->pc + imm : s->dnpc); btrace_write(4, s->pc, s->dnpc);//跳转指令
    INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu ,B,   s->dnpc = (src1 < src2)? s->pc + imm : s->dnpc); btrace_write(5, s->pc, s->dnpc);//跳转指令
    INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt , M,   R(rd) = ((int32_t)src1 <  (int32_t)src2)? 1 : 0);
    INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge , B,   s->dnpc = ((int32_t)src1 >= (int32_t)src2)? s->pc + imm : s->dnpc); btrace_write(6, s->pc, s->dnpc);//跳转指令
    INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu ,B,   s->dnpc = (src1 >= src2)? s->pc + imm : s->dnpc); btrace_write(7, s->pc, s->dnpc);//跳转指令
